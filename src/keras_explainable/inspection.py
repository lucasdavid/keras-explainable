...
def endpoints(model: Model, endpoints: List[E]) -> List[KerasTensor]:
  ...
  for ep in endpoints:
    if isinstance(ep, int):
      ...
      endpoint = (
        get_nested_layer(model, layer).get_input_at(index)
        if link == "input"
        else get_nested_layer(model, layer).get_output_at(index)
      )
  ...
  layers = [
    ...
    if (
      ...
      and (
        ...
        or hasattr(layer, 'bias') and layer.bias is not None
      )
    )
  ]
